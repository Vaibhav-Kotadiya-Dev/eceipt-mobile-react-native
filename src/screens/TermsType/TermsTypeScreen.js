...
import ZTermsTypeCard from '../../components/ZTermsTypeCard';
import { DataTable } from 'react-native-paper';
import ZText from '../../components/ZText';
import { useAllTermType, useDeleteTermType } from '../../services/TermsTypeService';
...
const TermsScreen = (props) => {  
  ...
  const { refetch, isFetching, isLoading } = useAllTermType(
    (response) => {
        ...
        <View style={styles.container}>
          {terms?.map((value) => (
            <ZTermsTypeCard 
              ...
              type={value?.type} 
              data={value}
              onPress={() => props.navigation.navigate("TermsTypeEdit", { term: value })}
