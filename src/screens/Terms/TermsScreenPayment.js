...
const TermsScreenPayment = (props) => {
  ...
  const { refetch, isFetching } = useAllTerms(
    (response) => {
        if (response.code === "SUCCESS") {
          const filterArr = response.data.data?.filter(i => i?.type === TERMS_TYPE.PAYMENT);
  ...
  return (
    <View style={styles.container} >
      ...
      <ScrollView showsVerticalScrollIndicator={false} style={{ flex: 1, paddingHorizontal: 8, paddingTop: 8 }} contentContainerStyle={{paddingBottom: 20}} refreshControl={<RefreshControl enabled={true} refreshing={refreshing} onRefresh={onRefresh} />}>
        {termList?.map((value) => (
          <ZTermsCard key={value?.id} type={TERMS_TYPE.PAYMENT} data={value}
...
export default TermsScreenPayment;
